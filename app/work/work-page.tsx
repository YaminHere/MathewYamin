...
export default function WorkPage({
  ...
  const [overflowingProjectTitles, setOverflowingProjectTitles] =
...
const applyTransform = () => {
 ...
 // --------------------------------------------------
  // PROJECT YEAR EDITING
  // --------------------------------------------------


  const handleProjectYearChange = (
  projectId: string,
  year: number
) => {
  setProjects((current) =>
...
const handleProjectResizePointerUp = (
 ...
 // --------------------------------------------------
  // PROJECT CLASSIFICATIONS EDITING
  // --------------------------------------------------


const handleProjectClassificationsChange = (
  projectId: string,
  classifications: string[]
) => {
  setProjects((current) =>
...
const handleFramePointerMove = (
  ...
  const projectScale =
    projectScales[drag.projectId] ?? 1;

  const scale =
    cameraScale * projectScale;

  const dx =
    (e.clientX - drag.startX) / scale;

  const dy =
    (e.clientY - drag.startY) / scale;
...
<div className="flex min-w-0 flex-1 items-center gap-4">
  ...
  <div
    className={`min-w-0 flex-[0_1_auto] overflow-hidden ${
      overflowingProjectTitles[project.id]
        ? "title-fade"
        : ""
    }`}
  >
  ...
  {/* CLASSIFICATIONS — FLEXIBLE, FIRST TO SHRINK */}
...
{editingProject === project.id && (
                  ...
                  onChange={(e) => {
                    ...
                    const next =
                      e.target.checked
                        ...
                        : project.classifications.filter(
                            (item) =>
                              item !==
                              classification
                          );

                    handleProjectClassificationsChange(
                      project.id,
                      next
                    );
                  }}
  ...
  e.stopPropagation();
    ...
    if (!isAdmin) return;
    if (!unlockedProjects[project.id]) return;
